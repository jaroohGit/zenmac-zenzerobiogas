...
// ── SQLite database ────────────────────────────────────────────────
const DB_PATH = process.env.DB_PATH || path_1.default.join(process.cwd(), "data", "kd_history.db");
...
function saveDailySummary() {
    try {
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
    `).run(isoDate, serumFlow, latexFlow, +(serumFlow + latexFlow).toFixed(1), tb1kwh, tb2kwh, +(tb1kwh + tb2kwh).toFixed(2), getRunHours('tb1'), getRunHours('tb2'), orpSerum, orpLatex, new Date().toISOString());
...
app.get("/api/kd/history/daily", (req, res) => {
    const month = String(req.query.month || "");
    if (!/^\d{4}-\d{2}$/.test(month))
        return res.status(400).json({ error: "invalid month, use YYYY-MM" });
    const rows = db.prepare("SELECT * FROM daily_summary WHERE date LIKE ? ORDER BY date ASC").all(`${month}-%`);
...
app.get("/api/kd/history/monthly", (req, res) => {
  ...
  `).all(`${year}-%`);
    ...
    const data = labels.map((label, idx) => {
        ...
        if (!r)
            return { label, month: idx + 1, serum: null, latex: null, flow: null, kwh: null, kwh1: null, kwh2: null, orp_serum: null, orp_latex: null };
        return {
            label, month: idx + 1,
...
mqttClient.on("message", (topic, message) => {
    ...
    try {
        ...
        if (live.serumSensor?.orp > 0) {
            orpAccum.serum.sum += live.serumSensor.orp;
            orpAccum.serum.count++;
        }
        if (live.processSensor?.orp > 0) {
            orpAccum.latex.sum += live.processSensor.orp;
            orpAccum.latex.count++;
        }
        io.emit("live", live);
